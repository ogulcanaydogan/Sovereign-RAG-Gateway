...
def test_chat_rag_sharepoint_managed_identity_mode(
    monkeypatch,
    tmp_path: Path,
    auth_headers: dict[str, str],
) -> None:
    ...
    monkeypatch.setenv("SRG_RAG_ALLOWED_CONNECTORS", "filesystem,sharepoint")
    monkeypatch.setenv("SRG_RAG_SHAREPOINT_SITE_ID", "site-id")
    monkeypatch.setenv("SRG_RAG_SHAREPOINT_AUTH_MODE", "managed_identity")
    monkeypatch.setenv("SRG_OPA_SIMULATE_TIMEOUT", "false")
    ...
    def fake_search_records(self, query: str):  # type: ignore[no-untyped-def]
        ...
        return [
            {
                "source_id": "sp-doc-mi",
                "uri": "https://contoso.sharepoint.com/sites/ops/Shared%20Documents/doc2",
                "text": "Managed identity runbook",
                "metadata": {
                    "name": "runbook-mi.md",
                    "path": "/drives/drive-1/root:/Ops/Runbooks",
                },
    ...
    monkeypatch.setattr(
        "app.rag.connectors.sharepoint.ManagedIdentityTokenProvider.get_token",
        lambda self: "managed-identity-token",  # noqa: ARG005
    )
    ...
    response = client.post(
        ...
        json={
            "model": "gpt-4o-mini",
            "messages": [{"role": "user", "content": "share managed identity runbook summary"}],
            "rag": {"enabled": True, "connector": "sharepoint", "top_k": 1},
        },
    )
    ...
    assert len(citations) == 1
    assert citations[0]["connector"] == "sharepoint"
    assert citations[0]["source_id"] == "sp-doc-mi"


def test_sharepoint_bearer_mode_requires_token(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("SRG_API_KEYS", "test-key")
    monkeypatch.setenv("SRG_AUDIT_LOG_PATH", str(tmp_path / "events.jsonl"))
    monkeypatch.setenv("SRG_RAG_ALLOWED_CONNECTORS", "filesystem,sharepoint")
    monkeypatch.setenv("SRG_RAG_SHAREPOINT_SITE_ID", "site-id")
    monkeypatch.setenv("SRG_RAG_SHAREPOINT_AUTH_MODE", "bearer_token")
    monkeypatch.delenv("SRG_RAG_SHAREPOINT_BEARER_TOKEN", raising=False)

    clear_settings_cache()
    with pytest.raises(RuntimeError, match="SRG_RAG_SHAREPOINT_BEARER_TOKEN"):
        create_app()
